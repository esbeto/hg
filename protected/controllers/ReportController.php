...
/* for print employee id card */	
	...
	public function actionStudentDocumentsearch()
	{
		...
		if(!empty($_POST['DocumentCategoryMaster']['document_category']))
		{
			...
			$this->redirect(array('studentdocumentsearchview','branch_id'=>$branch,'cat_id'=>$_POST['DocumentCategoryMaster']['document_category'],'acdm_period'=>$year,'sem'=>$sem));	
		}
...
//By Ravi Bhalodiya
     ...
     public function actionMyholidays()
     {
		...
		if(Yii::app()->user->getState('stud_id')){	
		   ...
		   $date_yr = date('Y');
		   $nat_holidays = NationalHolidays::model()->findAll(array('condition'=>'year(national_holiday_date)='.$date_yr.' and national_holiday_org_id='.$org,'order'=>'national_holiday_date'));
		}
